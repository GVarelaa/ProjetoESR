...
class Node:
    ...
    def sendback_worker(self, addr, msg):
        if len(self.database.neighbours != 1):
            next_step = msg.hops.pop()
            self.control_socket.sendto(Message(2, flag=2, data=msg.hops).serialize(), (next_step, 7777))
            self.logger.info(f"Control Service: Could not forward subscription message from {msg.hops[0]}. Sending back to {msg.hops[-1]}")
        else:
            exit()


    def leave_worker(self, msg):
        self.database.remove(msg.hops[0])
    ...
    def tracking_worker(self, msg):
        ...
        for neighbour in self.database.neighbours:
            forward_socket.sendto(msg.serialize(), (neighbour, 7777))

            self.logger.info(f"Control Service: Tracking message sent to neighbour {neighbour}")
